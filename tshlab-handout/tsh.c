...
eval(char *cmdline) 
{
    ...
    if(tok.builtins == BUILTIN_FG)
    {
      ...
      new_job = getjobjid(job_list, jobID);
      kill(-new_job->pid, SIGCONT);
    ...
    else if(tok.builtins == BUILTIN_BG)
    {
      ...
      new_job->state = BG;
      printf("[%d] (%d) %s\n", jobID, new_job->pid, new_job->cmdline);
    }
...
sigint_handler(int sig) 
{
  ...
  if(fg_pid)
    kill(-fg_pid, SIGINT); // kills the process
...
sigtstp_handler(int sig) 
{
  ...
  pid_t fg_pid = fgpid(job_list);
  if(fg_pid)
    kill(-fg_pid, SIGTSTP); // Stops the process
